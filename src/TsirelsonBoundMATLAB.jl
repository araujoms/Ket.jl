...
`scenario` is vector detailing the number of inputs and outputs, in the order [oa,ob,ia,ib], and `level` is an integer determining the level of the NPA hierarchy.
