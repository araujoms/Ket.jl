...
function seesaw(CG::Matrix{T}, scenario::Vector{<:Integer}, d::Integer) where {T <: Real}
    ...
    if oa == 2 && ob == 2
        ...
        B = [random_povm(T2, d, 2)[1] for _ in 1:ib]
        A = [Hermitian(zeros(T2, d, d)) for _ in 1:ia]
        local ψ
        ω = -R(Inf)
        i = 1
        while true
            ...
            if new_ω - ω <= minimumincrease || i > maxiter
                ...
                A = [[A[x]] for x in 1:ia] #rather inconvenient format
                B = [[B[y]] for y in 1:ib] #but consistent with the general case
                break
            end
            ω = new_ω
            i += 1
...
function _optimise_alice_assemblage(CG::Matrix{R}, scenario, B; solver = Hypatia.Optimizer{R}) where {R <: AbstractFloat}
    ...
    ρxa = [[JuMP.@variable(model, [1:d, 1:d] in JuMP.HermitianPSDCone()) for _ in 1:(oa - 1)] for _ in 1:ia] #assemblage
    ρ_B = JuMP.@variable(model, [1:d, 1:d], Hermitian) #auxiliary quantum state

    JuMP.@constraint(model, tr(ρ_B) == 1)
    for x in 1:ia
        JuMP.@constraint(model, ρ_B - sum(ρxa[x][a] for a in 1:(oa - 1)) in JuMP.HermitianPSDCone())
    end
    ...
    JuMP.optimize!(model)
    value_ρxa = [[Hermitian(JuMP.value.(ρxa[x][a])) for a in 1:(oa - 1)] for x in 1:ia]
...
function _optimise_bob_povm(CG::Matrix{R}, scenario, ρxa, ρ_B; solver = Hypatia.Optimizer{R}) where {R <: AbstractFloat}
    ...
    model = JuMP.GenericModel{R}()
    B = [[JuMP.@variable(model, [1:d, 1:d] in JuMP.HermitianPSDCone()) for _ in 1:(ob - 1)] for _ in 1:ib] #povm
    for y in 1:ib
        JuMP.@constraint(model, I - sum(B[y][b] for b in 1:(ob - 1)) in JuMP.HermitianPSDCone())
    end
    ...
    JuMP.set_silent(model)
    JuMP.optimize!(model)
    B = [[Hermitian(JuMP.value.(B[y][b])) for b in 1:(ob - 1)] for y in 1:ib]
...
function _compute_value_assemblage(CG::Matrix{R}, scenario, ρxa, ρ_B, B) where {R <: AbstractFloat}
    ...
    for a in 1:(oa - 1)
        for x in 1:ia
            tempB = sum(CG[aind(a, x), bind(b, y)] * B[y][b] for b in 1:(ob - 1) for y in 1:ib)
